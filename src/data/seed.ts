...
function generateHeatmap(): SpendingHeatmapDay[] {
  ...
  for (let i = 0; i < 365; i++) {
    ...
    data.push({
      ...
      amount: seededRandom(i + 1) > 0.1 ? amount : 0, // 10% zero days
    })
  }
  return data
}
...
function generateSparkline(base: number, volatility: number): number[] {
  ...
  let price = base
  for (let i = 0; i < 30; i++) {
    price += (Math.sin(i * 0.5) * volatility) + (seededRandom(base * 100 + i) - 0.48) * volatility
...
export const dailySpending: DailySpending[] = Array.from({ length: 30 }, (_, i) => {
  ...
  const amount = Math.round(base + Math.sin(i * 0.8) * 40 + seededRandom(700 + i) * 30)
